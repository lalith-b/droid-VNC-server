...
int initFlinger(void) {
	...
	int i, len;
	char lib_name[64];
	int apiLevel = get_most_preferrable_level();
	if(apiLevel == -1) {
		L("Could not get the api level, so cannot determine with version libdvnc_flinger_sdk.so to load\n");
		return -1;
	}
	sprintf(lib_name, DVNC_LIB_PATH "libdvnc_flinger_sdk%d.so",
			apiLevel);
	L("Try to load lib:%s\n", lib_name);
	// 1. Open lib
